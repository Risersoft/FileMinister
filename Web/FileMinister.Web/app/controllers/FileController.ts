...
class FileControllerGrid {

    constructor(public FileService: IFileService, public ngDialog: angular.dialog.IDialogService, public OrganizationService: IOrganizationService, public PermissionService: IPermissionService, public TagService: ITagService, public $scope: $Scope, public $rootScope: $RootScope, public $timeout: ng.ITimeoutService, public $compile: ng.ICompileService, public $filter: ng.IFilterService, public authInterceptorService: authInterceptorService) {

    }

    ReGenerateMainGridData(filter) {
        var that = this;
    ...
    InnerCellRenderer = function (params: any) {
        var image;
    ...
    menuOptionsTree = function (folderOrShare) {
        var that = this;
        ...
        return [
            ...
            null, // Dividier for Create Folder
            {
                text: 'Create Folder',
                enabled: function () {
    ...
    ClearRowSelection = function (event) {
        var that = this;
        $(event.api.gridPanel.eBodyViewport).off().on("click", function (e) {
            if ($(e.target).hasClass("ag-body-viewport")) {
    ...
    DownloadDeletedFile(selectedRow) {
        var that = this;
        //Get Azure download URL from Blob Name
...
class FileControllerDialogs extends FileControllerGrid {
    constructor(public FileService: IFileService, public ngDialog: angular.dialog.IDialogService, public OrganizationService: IOrganizationService, public PermissionService: IPermissionService, public TagService: ITagService, public $scope: $Scope, public $rootScope: $RootScope, public $timeout: ng.ITimeoutService, public $compile: ng.ICompileService, public $filter: ng.IFilterService, public authInterceptorService: authInterceptorService) {
        super(FileService, ngDialog, OrganizationService, PermissionService, TagService, $scope, $rootScope, $timeout, $compile, $filter, authInterceptorService);


    }


    openPermissionDialog(model: any) {
        var that = this;
        if (!model)
            model = this.$scope.gridOptions.api.getSelectedRows()[0];

        var permissionService: IPermissionService = this.PermissionService;
        var organizationService: IOrganizationService = this.OrganizationService;
        var filterService: ng.IFilterService = this.$filter;
        var showAlert = this.$scope.ShowAlert;

        permissionService.getEditPermission(model.value).then(
            (resp: any) => {
                if (resp && resp.data) {
                    ...
                    this.ngDialog.openConfirm({
                        ...
                        controller: ["$scope", function ($scope) {
                            ...
                            $scope.usersAndGroupsOptions = {
                                ...
                                onRowClicked: function (event) {
                                    ...
                                    if (loweredEventTargetTagName !== "button" && loweredEventTargetTagName !== "i") {
                                        ...
                                        var setPermissions = function (resp: any, isExisting: boolean = false) {
                                            //console.log(resp);
                                            if (isExisting) {
                                                ...
                                                var effectivePermissionsSources: any = [];
                                        ...
                                        if (userOrGroupPermissions && userOrGroupPermissions.userOrGroupPermissions && userOrGroupPermissions.userOrGroupPermissions.userOrGroup && userOrGroupPermissions.userOrGroupPermissions.permissions) {
                                            ...
                                            switch (userOrGroup.Type) {
                                                case 1:
                                                    permissionService.getPermissionByFileAndUser(model.value, userOrGroup.Id).then(
                                                        setPermissions,
                                                        (resp: any) => {
                                                            console.log(resp);
                                                        }
                                                    ...
                                                    break;
                                                case 2:
                                                    permissionService.getPermissionByFileAndGroup(model.value, userOrGroup.Id).then(
                                                        setPermissions,
                                                        (resp: any) => {
                                                            console.log(resp);
                                                        }
                            ...
                            $scope.addUserOrGroupRow = function (newlyAddedUserOrGroup) {
                                ...
                                var existingUserOrGroup: any = filterService('filter')($scope.usersAndGroups, { Id: newlyAddedUserOrGroup.Id, Type: newlyAddedUserOrGroup.Type }, true)[0];
                                ...
                                that.ngDialog.openConfirm({
                                    ...
                                    controller: ["$scope", function ($scope) {
                                        ...
                                        $scope.CheckNames = function () {
                                            ...
                                            organizationService.GetAccountGroupUsers(that.$rootScope.selectedAccount.accountId, $scope.userOrGroupName).then(
                                                (resp: any) => {
                                                    //console.log(resp);
                            ...
                            $scope.Save = function () {
                                ...
                                permissionService.savePermissionByFileAndGroup($scope.selectedFile, { AddedUsersAndGroupsPermissions: addedUsersAndGroupsPermissions, UpdatedUsersAndGroupsPermissions: updatedUsersAndGroupsPermissions, RemovedUsersAndGroupsPermissions: removedUsersAndGroupsPermissions }).then((resp: any) => {
                                    if (resp && resp.data) {
                                        if (resp.data.Data) {
                                            $scope.closeThisDialog();
                            ...
                            permissionService.getUsersAndGroupsByFileId(model.value).then((resp: any) => {
                                //console.log(resp);
                                $scope.usersAndGroups = resp.data.Data;
                                $scope.removedUsersAndGroupsPermissions = [];
                                $scope.usersAndGroupsPermissions = {};
    ...
    openTagDialog(model: any) {
        var that = this;
        if (!model)
            model = this.$scope.gridOptions.api.getSelectedRows()[0];

        var tagService: ITagService = this.TagService;
        var permissionservice: IPermissionService = this.PermissionService;

        var showAlert = this.$scope.ShowAlert;

        permissionservice.getEditTagPermission(model.value).then(
            (resp: any) => {
                if (resp && resp.data) {
                    ...
                    this.ngDialog.openConfirm({
                        ...
                        controller: ["$scope", function ($scope) {
                            ...
                            $scope.Save = function () {
                                if (isValid()) {
                                    ...
                                    if (duplicateKeies.length > 0) {
                                        ...
                                        tagService.save($scope.selectedFile, { AddedTags: addedTags, UpdatedTags: updatedTags, RemovedTags: $scope.removedTags }).then((resp: any) => {
                                            if (resp && resp.data) {
                                                if (resp.data.Data) {
                                                    $scope.closeThisDialog();
                            ...
                            tagService.getByFileId(model.value).then((resp: any) => {
                                //console.log(resp);
                                setTimeout(function () {
    ...
    openAccountSelectionDialog(switchAccount: boolean) {
        var that = this;
        ...
        dialog.closePromise.then((data: any) => {
            if (data && !data.value)
                return;
    ...
    syncData() {
        ...
        that.FileService.SyncServerData().then((resp: any) => {
            if (resp) {
                //Load file/Share list
                that.FileService.GetAllUserShares().then((resp: any) => {
    ...
    OpenCreateFolderDialog(selectedNode: any = null) {
        var that = this;
        if (this.$scope.gridOptions.api.getSelectedRows().length <= 0)
            return;
    ...
    OpenRenameDialog = function (fileEntryId: any, versionNumber: any, newName: string, folder: any) {
        var that = this;
        if (this.$scope.gridOptions.api.getSelectedRows().length <= 0)
            return;
    ...
    OpenMoveDialog = function (model: any) {
        var that = this;
        ...
        var getChildren = function (folderId) {
            return fileService.GetAllChildren({ id: folderId, filter: null }).then((resp: any) => {
        ...
        if (shares && shares.length > 0) {
            shares = [shares[0]];
            shares[0].open = true;
            getChildren(shares[0].value).then((folderChildren: any) => {
                ...
                that.ngDialog.openConfirm({
                    ...
                    controller: ["$scope", function ($scope) {
                            ...
                            onRowClicked: function (event) {
                                ...
                                if (event.event.target.tagName.toLowerCase() === "i") {
                                    ...
                                    if (event.node.expanded) {
                                        getChildren(selectedChildFolder.value).then((folderChildren: any) => {
                        ...
                        $scope.Move = function () {
                            ...
                            var moveFile = function (isReplaceExistingFile: boolean = false, newFileName: string = "") {
                                fileService.Move(model.value, selectedChildFolder.value, isReplaceExistingFile, newFileName).then((resp: any) => {
                                    if (resp && resp.data) {
                                        if (resp.data.Data) {
                                            $scope.closeThisDialog();
    ...
    DeleteRestorePurgeDialog = function (model: any, action: string, selectedNode: any = null) {
        var that = this;
        ...
        that.ngDialog.openConfirm({
            ...
            controller: ["$scope", function ($scope) {
                ...
                $scope.Action = function (action) {
                    ...
                    if (actionMethod) {
                        actionMethod.call(fileService, fileList).then((resp: any) => {
                            if (resp && resp.data) {
                                if (resp.data.Data) {
                                    $scope.closeThisDialog();
    ...
    OpenConfirmDialog = function (bodyText: string, titleText: string, callback: any) {
        var that = this;
        var confirmDialog = that.ngDialog.openConfirm({
            ...
            controller: ['$scope', '$window', function ($scope, $window) {
                ...
                $scope.confirm = function (data: any) {
                    if (data && !data.value)
                        return;
                    if (callback)
                        callback.apply();
    ...
    OpenDeletedFileDialog = function () {
        var that = this;
        if (this.$scope.gridOptions.api.getSelectedRows().length <= 0)
            return;
        var selectedFolderId = this.$scope.gridOptions.api.getSelectedRows()[0].value;
            ...
            controller: ['$scope', '$window', function ($scope, $window) {
                ...
                var deletedFileColumnDefs = [
                    ...
                    { headerName: "Deleted On", field: "DeletedOn", width: 150 },
                    {
                        ...
                        cellRenderer: function (params) {
                            var buttonStr =
                                '<button id="btn" class="btn btn-danger btn-xs" title= "Permanent Delete" ' + " ng-click='DeleteFileGridActionClick(\"PermanentDelete\", $event, \"" + params.data.value + "\")' >" +
                ...
                $scope.DeleteFileGridActionClick = function (action, e, fileId) {
                    ...
                    if (action === "PermanentDelete") {
                        that.OpenConfirmDialog("Are you sure you want to permanently delete this file?", null,
                            function () {
...
class FileController$Scope extends FileControllerDialogs {
    ...
    constructor(public FileService: IFileService, public ngDialog: angular.dialog.IDialogService, public OrganizationService: IOrganizationService, public PermissionService: IPermissionService, public TagService: ITagService, public $scope: $Scope, public $rootScope: $RootScope, public $timeout: ng.ITimeoutService, public $compile: ng.ICompileService, public $filter: ng.IFilterService, public authInterceptorService: authInterceptorService) {
        ...
        $rootScope.setUser = (user) => {
            ...
            //debugger;
            PermissionService.isAccountAdmin().then(
                (resp: any) => {
                    if (resp && resp.data) {
        ...
        this.$scope.split_bar_mousedown = function (e) {
            e.preventDefault();
            var grid = this;
        ...
        this.$scope.ShowAlert = function (msg: string, alertType: any) {
        ...
        this.$scope.openHistoryDialog = function (id: string) {
            var dialog = that.ngDialog.openConfirm({
                ...
                controller: ['$scope', function ($scope) {
                    ...
                    that.FileService.GetVersionHistory(id)
                        .then((resp: any) => {
                            ...
                            angular.forEach(resp.data.Data, function (item, index) {
                                historyRowData.push({
                                    name: item["FileEntryNameWithExtension"],// FileEntryName
                                    value: item["FileEntryId"],
        ...
        this.$scope.UndoCheckout = function () {
        ...
        this.$scope.buttonClick = function (key, e) {
...
class FileController extends FileController$Scope implements angular.IController {
    static $ = ['JQuery'];
    $onInit() { }
    constructor(public FileService: IFileService, public ngDialog: angular.dialog.IDialogService, public OrganizationService: IOrganizationService, public PermissionService: IPermissionService, public TagService: ITagService, public $scope: $Scope, public $rootScope: $RootScope, public $timeout: ng.ITimeoutService, public $compile: ng.ICompileService, public $filter: ng.IFilterService, public authInterceptorService: authInterceptorService) {
        ...
        //$scope.IsAdmin = true;
        $rootScope.setSelectedAccount = (account) => {
            try {
                //debugger;
        ...
        this.OrganizationService.GetUserLogedIn()
            .then((resp: any) => {
                if (resp.data.Data) {
                    this.$rootScope.isUserAuthentic = true;
                    this.setAccount();
        ...
        this.$scope.IsFileORFolderSelected = function () {
            return (this.maingridOptions.api.getSelectedRows().length > 0);
        }

        this.$scope.IsShareSelected = function () {
        ...
        this.$scope.IsFileSelected = function () {
        ...
        this.$scope.IsOnlyOneFileSelected = function () {
            var selectedRows = this.maingridOptions.api.getSelectedRows();
            var files = selectedRows.filter(item => { return !item.folder && item.version > 0; });

            return selectedRows.length === 1 && selectedRows.length === files.length;
        }

        this.$scope.IsFileAndFolderSelected = function () {
            var folders = this.maingridOptions.api.getSelectedRows().filter(item => { return item.folder === true; });
        ...
        this.$scope.IsCheckedOut = function () {
            if (this.maingridOptions.api.getSelectedRows().length == 0)
                return false;
            var item = this.maingridOptions.api.getSelectedRows()[0];
        ...
        this.$scope.IsAuthorizedToUndoCheckedOut = function () {
        ...
        this.$scope.IsFolderSelected = function () {
            if (this.gridOptions.api.getSelectedRows().length == 0)
                return false;
            return !(this.gridOptions.api.getSelectedRows()[0].CanWrite === false);
        }

        this.$scope.IsBlobContainerSelected = function () {
            if (this.gridOptions.api.getSelectedRows().length == 0)
                return false;
            return true;
        }

        this.$scope.Filter = { tags: null, SearchText: "", IsAdvancedSearch: false };
        this.$scope.searchChanged = false;

        this.$scope.SearchChange = function () {
            that.$scope.searchChanged = true;
        }

        this.$scope.FileSearch = function () {
    ...
    getSelectedFolderModel = function () {
    ...
    AttachDropzone() {
        var that = this;
        var fileUploadUrl: string = "/file/post";
    ...
    setAccount() {
        this.OrganizationService.GetAccountName()
            .then((resp: any) => {
                if (resp.data.Data) {
                    let acct = { accountId: resp.data.Data.AccountId, accountName: resp.data.Data.AccountName };
                    ...
                    //debugger
                    this.$rootScope.isUserAuthentic = true;
                    this.openAccountSelectionDialog(false);
                }
            },
                (resp: any) => {
                    console.log(resp);
