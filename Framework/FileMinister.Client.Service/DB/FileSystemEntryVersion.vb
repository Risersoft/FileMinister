'------------------------------------------------------------------------------
' <auto-generated>
'     This code was generated from a template.
'
'     Manual changes to this file may cause unexpected behavior in your application.
'     Manual changes to this file will be overwritten if the code is regenerated.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic

Partial Public Class FileSystemEntryVersion
    Public Property FileSystemEntryVersionId As System.Guid
    Public Property ParentFileSystemEntryId As System.Guid
    Public Property FileSystemEntryId As System.Guid
    Public Property VersionNumber As Nullable(Of Integer)
    Public Property PreviousFileSystemEntryVersionId As Nullable(Of System.Guid)
    Public Property FileSystemEntrySize As Long
    Public Property FileSystemEntryName As String
    Public Property FileSystemEntryExtension As String
    Public Property FileSystemEntryRelativePath As String
    Public Property FileSystemEntryHash As String
    Public Property ServerFileSystemEntryName As Nullable(Of System.Guid)
    Public Property CreatedOnUTC As Date
    Public Property CreatedByUserId As System.Guid
    Public Property IsDeleted As Boolean
    Public Property DeletedOnUTC As Nullable(Of Date)
    Public Property DeletedByUserId As Nullable(Of System.Guid)
    Public Property FileSystemEntryNameWithExtension As String

    Public Overridable Property DeltaOperations As ICollection(Of DeltaOperation) = New HashSet(Of DeltaOperation)
    Public Overridable Property FileSystemEntry As FileSystemEntry
    Public Overridable Property FileSystemEntry1 As FileSystemEntry
    Public Overridable Property FileSystemEntryVersionConflicts As ICollection(Of FileSystemEntryVersionConflict) = New HashSet(Of FileSystemEntryVersionConflict)
    Public Overridable Property FileSystemEntryVersions1 As ICollection(Of FileSystemEntryVersion) = New HashSet(Of FileSystemEntryVersion)
    Public Overridable Property FileSystemEntryVersion1 As FileSystemEntryVersion

End Class
